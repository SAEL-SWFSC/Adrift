...
```{r, PASCALdeployExport2, include=FALSE}
#Change Column Labels, decimal places 
 
DeployTable_PASCAL_ex <- deployDetails %>%
  gt()%>%
...
DeployTable_PASCAL_ex  
gtsave(DeployTable_PASCAL_ex, filename = here("output", "DeployTable_PASCAL.html"))
...
```{r, Complete Deployment Table Pascal}
# Make Expanded Table for Appendix
DeployTable_Pascal <- deployDetails %>% 
  gt()%>%
  tableTheme()%>%
  # tab_header('PASCAL Survey') %>%
  sub_missing(missing_text = "--")%>%
  tab_spanner(
    label = "Deployment",
